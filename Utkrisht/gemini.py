...
def text_to_notes(transcript):
    ...
    prompt =  '''Format this professionally in markdown, with headings, bolded keywords, and bullet points. Example : graph TD;
            CO2[CO2 + H2O] -->|Light Energy| Chlorophyll[Chlorophyll];
            Chlorophyll -->|Photosynthesis| Glucose[Glucose + O2]; '''
...
def text_to_flowchart(transcript):
    multimodal_model = genai.GenerativeModel("gemini-1.0-pro")
    response = multimodal_model.generate_content(
        [transcript, "Format a flowchart description for mermaid.js to draw flowchart and include headings and important points."]
    )
    html_text = markdown.markdown(response.text)
    return html_text
